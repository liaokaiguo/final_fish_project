<template>
  <div class="background">
    <div>
      <el-row>
        <el-col :span="24">
          <div class="centTitle">刺网作业方式统计分析</div>
      ...
      <el-row>
        ...
        <div class="mainEchartsStyle" id="gillEchartsId"></div>
      </el-row>
    </div>
  </div>
</template>
...
<script>
// gill net(刺网) fishing vessel statistic and analysis
export default {
  data() {
    return {
      gillOption: {
  ...
  methods: {
    ...
    drawGillCharts() {
      var gillEChart = this.$echarts.init(
        document.getElementById("gillEchartsId")
      );
      gillEChart.setOption(this.gillOption);
        //数据没有加载出来显示加载动画,样式添加todo
        gillEChart.showLoading();
        //获取数据
        this.axios.get('/getCiwangStatistic').then(res => {
            gillEChart.hideLoading(); //加载出来隐藏加载动画
            gillEChart.setOption({  //数据添加
                series: [{
